...
def create(src, dst, metadata=b""):
    """BPS with SourceRead, SourceCopy (block matches anywhere in source), TargetCopy (byte runs)."""
    out = bytearray(b"BPS1")
    out += _num(len(src)) + _num(len(dst)) + _num(len(metadata)) + metadata
    BLOCK = 32
    index = {}
    for k in range(0, len(src) - BLOCK + 1, BLOCK):
        index.setdefault(src[k:k + BLOCK], k)
    n, i = len(dst), 0
    src_rel = dst_rel = 0
    lit_start = 0

    def rel(off):
        return _num((abs(off) << 1) | (1 if off < 0 else 0))

    def flush(upto):
        nonlocal lit_start
        if upto > lit_start:
            out.extend(_num(((upto - lit_start - 1) << 2) | TARGET_READ) + dst[lit_start:upto])
        lit_start = upto

    while i < n:
        # 1) identical at the same offset
        if i < len(src) and src[i] == dst[i]:
            ...
            if j - i >= 8:
                ...
                i = j; lit_start = i
                continue
        # 2) byte run -> TargetCopy of previous byte
        if i > 0 and dst[i] == dst[i - 1]:
            j = i
            while j < n and dst[j] == dst[i - 1]:
                j += 1
            if j - i >= 12:
                flush(i)
                out += _num(((j - i - 1) << 2) | TARGET_COPY) + rel((i - 1) - dst_rel)
                dst_rel = i - 1 + (j - i)
                i = j; lit_start = i
                continue
        # 3) block found elsewhere in source -> SourceCopy
        blk = dst[i:i + BLOCK]
        k = index.get(blk) if len(blk) == BLOCK else None
        if k is not None and k != i:
            j = i + BLOCK; kk = k + BLOCK
            while j < n and kk < len(src) and src[kk] == dst[j]:
                j += 1; kk += 1
            flush(i)
            out += _num(((j - i - 1) << 2) | SOURCE_COPY) + rel(k - src_rel)
            src_rel = kk
            i = j; lit_start = i
            continue
        i += 1
